...
-- Create function to safely process subscription charge with advisory lock
...
begin
  -- Try to obtain advisory lock using order ID hash
  ...
  -- Mark order as processed first to prevent race conditions
  update orders
  ...
  exception 
    when others then
      -- Log error but don't fail since order is already marked as processed
